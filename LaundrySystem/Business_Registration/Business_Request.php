...
try {
    ...
    if (isset($_FILES['Owner_File']) && $_FILES['Owner_File']['error'] === UPLOAD_ERR_OK) {
        ...
        $stmtpic = $conn->prepare("INSERT INTO request_bsOwner (rqbs_owner, rqbsowner_filenm, mime, size, data) VALUES (?, ?, ?, ?, ?)");
        if (!$stmtpic) {
            throw new Exception('Prepare failed for request_bsOwner: ' . $conn->error);
        }
        $stmtpic->bind_param("sssib", $OwnerName, $pic_name, $mime, $size, $data);
        $stmtpic->send_long_data(4, $data);
        if (!$stmtpic->execute()) {
    ...
    if (isset($_FILES['B_logo']) && $_FILES['B_logo']['error'] === UPLOAD_ERR_OK) {
        ...
        $stmtLogo = $conn->prepare("INSERT INTO request_bsLogo(rqbs_ID, rqbslogo_name, mime, size, data) VALUES (?, ?, ?, ?, ?)");
        if (!$stmtLogo) {
            throw new Exception('Prepare failed for request_bsLogo: ' . $conn->error);
        }
        $stmtLogo->bind_param("issib", $rqbspic_ID, $logo_name, $mime, $size, $data);
        $stmtLogo->send_long_data(4, $data);
        if (!$stmtLogo->execute()) {
    ...
    if (isset($_FILES['DTI_File']) && $_FILES['DTI_File']['error'] === UPLOAD_ERR_OK) {
        ...
        $stmtDTI = $conn->prepare("INSERT INTO request_bsDTI(rqbsDTI_ID, rqbsDTI_name, rqbsDTI_No, mime, size, data) VALUES (?, ?, ?, ?, ?, ?)");
        if (!$stmtDTI) {
            throw new Exception('Prepare failed for request_bsDTI: ' . $conn->error);
        }
        $stmtDTI->bind_param("isssib", $rqbspic_ID, $DTI_name, $DTI_no, $mime, $size, $data);
        $stmtDTI->send_long_data(5, $data);
        if (!$stmtDTI->execute()) {
    ...
    if (isset($_FILES['TIN_File']) && $_FILES['TIN_File']['error'] === UPLOAD_ERR_OK) {
        ...
        $stmtTIN = $conn->prepare("INSERT INTO request_bsTIN(rqbsTIN_ID, rqbsTIN_name, rqbsTIN_No, mime, size, data) VALUES (?, ?, ?, ?, ?, ?)");
        if (!$stmtTIN) {
            throw new Exception('Prepare failed for request_bsTIN: ' . $conn->error);
        }
        $stmtTIN->bind_param("isssib", $rqbspic_ID, $TIN_name, $TIN_no, $mime, $size, $data);
        $stmtTIN->send_long_data(5, $data);
        if (!$stmtTIN->execute()) {
    ...
    if (isset($_FILES['B_Sign']) && $_FILES['B_Sign']['error'] === UPLOAD_ERR_OK) {
        ...
        $stmtAgreement = $conn->prepare("INSERT INTO request_bsAgreement(rqbs_ID, rqbs_owner, rqbsSign_name, mime, size, data) VALUES (?, ?, ?, ?, ?, ?)");
        if (!$stmtAgreement) {
            throw new Exception('Prepare failed for request_bsAgreement: ' . $conn->error);
        }
        $stmtAgreement->bind_param("isssib", $rqbspic_ID, $OwnerName, $Sign_name, $mime, $size, $data);
        $stmtAgreement->send_long_data(5, $data);
